// ============================================================
// SEREIN · Subida automática de facturas de compra a Google Drive
// Supabase Edge Function — subir-factura-compra-drive
// ============================================================
// Recibe el PDF de una factura de compra (la que sube la persona al
// registrar una "Compra imputada" en Módulo de Proyectos) como base64, y
// lo sube a la misma Unidad compartida que ya usan los protocolos de
// calidad, organizado en una rama propia: COMPRAS / <OT> / <Centro de
// Costo> / archivo.pdf. Mismo mecanismo exacto que
// subir-protocolo-drive (cuenta de servicio, JWT firmado, carpetas
// encadenadas buscar-o-crear) — se duplica en vez de compartir código
// porque cada Edge Function de este proyecto se despliega como archivo
// autocontenido.
//
// Requiere los secretos:
//   GOOGLE_SERVICE_ACCOUNT_JSON  (el JSON completo de la cuenta de servicio)
//   DRIVE_ROOT_FOLDER_ID         (la Unidad compartida raíz — la misma que
//                                 usa subir-protocolo-drive)
// ============================================================
...
// Busca una subcarpeta por nombre dentro de un padre dado; si no existe,
// la crea. Se encadena para armar COMPRAS / OT / Centro de Costo. driveId
// siempre es el id de la Unidad compartida raíz (no el de la carpeta
// padre inmediata) — lo exige la API para buscar/crear dentro de una
// Unidad compartida.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { pdfBase64, filename, ot, centroCosto } = await req.json();
    ...
    const carpetaCompras = await buscarOCrearCarpeta(token, rootId, rootId, "COMPRAS");
    const carpetaOT = await buscarOCrearCarpeta(token, rootId, carpetaCompras, ot || "Sin OT");
    const folderId = await buscarOCrearCarpeta(token, rootId, carpetaOT, centroCosto || "Sin centro de costo");
    ...
    console.error("subir-factura-compra-drive error:", err);
