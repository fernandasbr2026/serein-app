...
const T = THEME
const CASOS_DIF = { A: 'Planchas, estanques exteriores, vigas simples, superficies amplias y accesibles', B: 'Perfiles estructurales, columnas, algo de interior, acceso regular', C: 'Reticulados / celosÃ­as, muchas aristas o sectores, mayormente interior', D: 'Ductos, interior confinado, caÃ±erÃ­as, geometrÃ­a difÃ­cil', E: 'Confinado + geometrÃ­a difÃ­cil + interior simultÃ¡neamente' }
...
export default function CotizadorCalculo({ clientes = [], onAddCliente = () => {}, cotizaciones = [], setCotizaciones = () => {}, onVolver = () => {} }) {
  ...
  function guardar() {
    ...
    const cot = { id: 'cot' + Date.now(), numero, folio: numero, area: sede, vencimiento: new Date().toISOString().slice(0, 10), tipo: 'calculo', origen: 'cotizador', estado: 'Alta probabilidad de cierre', cliente: cli.nombre || '', rut: cli.rut || '', giro: cli.giro || '', direccion: cli.direccion || '', comuna: cli.comuna || '', ciudad: cli.ciudad || cli.comuna || '', condicionPago: 'CONTADO', vendedor: cli.vendedor || 'Mario Vidal', sede, fecha: new Date().toISOString().slice(0, 10), margenVenta: +pct, porcentajeGanancia: +pct,
      items: items.map((it, i) => { const d = dg(it); const pm = precioMargen(d.costoM2, pct); return { codigo: it.grado || '', detalle: (it.desc || 'Item ' + (i + 1)) + (it.ral ? ' - ' + it.ral : '') + ' - ' + (it.capas.filter(c => c.p).map(c => c.p).join(' + ') || 'solo granallado ' + it.grado), cant: +it.m2 || 0, unidad: 'mÂ²', pUnitario: Math.round(pm), descuento: 0, comentario: (it.capas.filter(c => c.p).map(c => c.p + ' ' + milsProm(c) + ' mils').join(' + ') || 'Solo granallado') + ' - ' + it.grado, descripcion: it.desc, ral: it.ral, m2: +it.m2 || 0, gradoSSPC: it.grado, factorDificultad: it.dif, limpiezaSP1: +it.limpieza || 0, capas: it.capas.filter(c => c.p), costoM2: Math.round(d.costoM2), precioM2: Math.round(pm), total: Math.round(pm * (+it.m2 || 0)), desglose: { granallado: Math.round(d.granallado), aplicacion: Math.round(d.aplicacion), diluyente: Math.round(d.diluyente), pintura: Math.round(d.pintura), fijos: Math.round(d.fijos) } } }),
    ...
    {items.map((it, i) => { const d = dg(it); const pm = precioMargen(d.costoM2, pct); const soloGran = d.nCapas === 0; return (
      <div key={i} style={card}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8, marginTop: 6 }}>
          ...
          <div style={{ fontSize: 12.5, color: T.textSoft }}>Costo pintura: <b style={{ color: T.navy }}>{clp(d.pintura)}/m2</b> - {d.nCapas} capas Â· mils promedio por rango</div>
        </div>
