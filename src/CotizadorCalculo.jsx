...
export default function CotizadorCalculo({ clientes = [], onAddCliente = () => {}, cotizaciones = [], setCotizaciones = () => {}, onVolver = () => {} }) {
  ...
  function guardar() {
    ...
    const cot = { numero, folio: numero, tipo: 'calculo', origen: 'cotizador', estado: 'Cotizada', cliente: cli.nombre, clienteRut: cli.rut || '', clienteDireccion: cli.direccion || '', sede, fecha: new Date().toISOString().slice(0, 10), porcentajeGanancia: +pct,
      items: items.map(it => { const d = dg(it); const pm = precioM2(d.costoM2, pct); return { descripcion: it.desc, ral: it.ral, m2: +it.m2 || 0, gradoSSPC: it.grado, factorDificultad: it.dif, limpiezaSP1: +it.limpieza || 0, capas: it.capas.filter(c => c.p), costoM2: Math.round(d.costoM2), precioM2: Math.round(pm), total: Math.round(pm * (+it.m2 || 0)), desglose: { granallado: Math.round(d.granallado), aplicacion: Math.round(d.aplicacion), diluyente: Math.round(d.diluyente), pintura: Math.round(d.pintura), fijos: Math.round(d.fijos) } } }),
    ...
    {items.map((it, i) => { const d = dg(it); const pm = precioM2(d.costoM2, pct); return (
      <div key={i} style={card}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
          <span style={{ fontSize: 12, fontWeight: 600, color: T.textSoft }}>ESQUEMA POR CAPA</span>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 720 }}>
            ...
            <tbody>
              {it.capas.map((c, j) => { const prod = idx[c.p]; const mp = milsProm(c); const rend = prod ? rendimientoM2Gal(prod.s, mp, c.perdida, cte.constante) : 0; const vg = prod ? valorGalon(prod, cte.litrosPorGalon) : 0; const vm = prod ? valorM2Capa(prod, mp, c.perdida, cte) : 0; return (<tr key={j}>
                ...
                <td style={tdc}><select value={c.p} onChange={e => updItem(i, x => x.capas[j].p = e.target.value)} style={{ ...inp, width: '100%', minWidth: 150, padding: '5px 7px' }}><option value="">- producto -</option>{P.productos.map((pr, k) => <option key={k} value={pr.n}>{pr.n}</option>)}</select></td>
        ...
        <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'flex-start', borderTop: '1px solid ' + T.borderSoft, paddingTop: 10, marginTop: 10 }}>
          <div style={{ minWidth: 200, flex: 1 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11.5, color: T.textSoft, padding: '1px 0' }}><span>Granallado /m2</span><span>{clp(d.granallado)}</span></div>
