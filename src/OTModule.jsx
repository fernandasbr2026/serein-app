...
function nuevoProtocolo(tipo, ot, correlativo, instrumentos) {
  ...
  const base = { id: 'pr' + Date.now() + Math.floor(Math.random() * 999), tipo, correlativo, codigo: tipo + ' ' + cod, pgpCodigo: 'PGP ' + cod, docNro: tipo === 'PIG' ? 'RC-GP-1' : 'RC-PG-6', ot: ot.numero || '', area: ot.area || '', oc: ot.oc || '', nv: ot.nv || '', cliente: ot.cliente || '', proyecto: '', esquemaProyecto: esquemaOt, preparadoPor: 'Boris Gomez', revisadoPor: 'Luis Soto', aprobadoPor: 'Luis Soto', fecha: h, marcas: [], logoCliente: '', firmas: [{ rol: 'Aprobado', quien: 'Luis Soto', fecha: '' }, { rol: 'Recepcionado', quien: 'Cliente', fecha: '' }, { rol: 'Aprobado', quien: 'Inspector Cliente', fecha: '' }] }
...
function rptInfo(pairs, equipos) { var s = '<div class="infogrid">'; pairs.forEach(function (pr) { var img = FILAS_CON_FIRMA.indexOf(pr[0]) >= 0 ? firmaImgPara(equipos, pr[1]) : ''; s += '<div class="info-item"><span class="info-k">' + pr[0] + '</span><span class="info-v" style="display:flex;align-items:center;justify-content:flex-end;gap:6px">' + esc(pr[1]) + (img ? '<img src="' + img + '" style="height:44px;max-width:110px;object-fit:contain"/>' : '') + '</span></div>' }); return s + '</div>' }
...
function rptSign(firmas, equipos) { var r = ''; (firmas || []).forEach(function (f) { var img = firmaImgPara(equipos, f.quien); r += '<tr><td>' + esc(f.rol) + '</td><td>' + esc(f.quien) + '</td><td class="sign-cell" style="overflow:visible">' + (img ? '<img src="' + img + '" style="height:50px;max-width:140px;object-fit:contain;display:block;margin:-9px auto"/>' : '') + '</td><td>' + esc(f.fecha) + '</td></tr>' }); return '<table class="dt"><thead><tr><th>Rol</th><th>Nombre</th><th>Firma</th><th>Fecha</th></tr></thead><tbody>' + r + '</tbody></table>' }
