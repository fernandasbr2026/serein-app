...
export function nuevoProtocolo(tipo, ot, correlativo, instrumentos) {
...
function rptEquipos(eq) { eq = eq || {}; var cols = [['Medidor de espesor', eq.espMarca || 'ELCOMETER', eq.espSerie || '', eq.espFotos || []], ['Rugosimetro', eq.rugMarca || 'ELCOMETER', eq.rugSerie || '', eq.rugFotos || []], ['Termohigrometro', eq.termoMarca || 'ELCOMETER', eq.termoSerie || '', eq.termoFotos || []], ['Equipo de adherencia', eq.adhMarca || '', eq.adhSerie || '', eq.adhFotos || []]]; var s = '<div class="equipos">'; cols.filter(function (c, k) { return k < 3 || c[1] || c[2] || (c[3] || []).length }).forEach(function (c) { var ig = ''; (c[3] || []).forEach(function (d) { ig += '<div class="eq-img"><img src="' + d + '"/></div>' }); s += '<div class="eq-col"><div class="eq-name">' + esc(c[1]) + '</div><div class="eq-code">' + esc(c[2]) + '</div><div class="eq-sub">' + esc(c[0]) + '</div>' + ig + '</div>' }); return s + '</div>' }
...
export function htmlDeProtocolo(p, equipos) { return p.tipo === 'PIG' ? htmlPIG(p, equipos) : p.tipo === 'EA' ? htmlEA(p, equipos) : htmlPGP(p, equipos) }
