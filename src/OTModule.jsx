...
function nuevoProtocolo(tipo, ot, correlativo, instrumentos) {
  ...
  const base = { id: 'pr' + Date.now() + Math.floor(Math.random() * 999), tipo, correlativo, codigo: tipo + ' ' + cod, pgpCodigo: 'PGP ' + cod, docNro: tipo === 'PIG' ? 'RC-GP-1' : 'RC-PG-6', ot: ot.numero || '', area: ot.area || '', oc: ot.oc || '', nv: ot.nv || '', cliente: ot.cliente || '', proyecto: '', preparadoPor: 'Boris Gomez', revisadoPor: 'Luis Soto', aprobadoPor: 'Luis Soto', fecha: h, marcas: [], logoCliente: '', firmas: [{ rol: 'Aprobado', quien: 'Tecnico Pintura', fecha: '' }, { rol: 'Recepcionado', quien: 'Cliente', fecha: '' }, { rol: 'Aprobado', quien: 'Inspector Cliente', fecha: '' }] }
...
function htmlPIG(p, equipos) {
  ...
  var page2 = ''; if (evChecks.length) page2 = '<div class="page">' + rptHeader('PROTOCOLO INICIO DE GRANALLA', 'ANEXO IMAGENES EVIDENCIA', [['Documento N', p.docNro || ''], ['Orden de Trabajo', p.ot || ''], ['Pagina', '2/2']], p.logoCliente) + rptInfo(info2) + rptSection('Anexo imagenes evidencia') + ev + rptSection('Firmas') + rptSign(p.firmas) + rptFooter() + '</div>';
...
function htmlPGP(p, equipos) {
  ...
  var info2 = [['Preparado por', p.preparadoPor], ['Revisado por', p.revisadoPor], ['Aprobado por', p.aprobadoPor], ['Cliente', p.cliente], ['Proyecto', p.proyecto], ['Prot. Granallado', p.pgpCodigo], ['Prot. Pintura', p.pgpCodigo], ['Fecha', p.fecha]];
  var page2 = ''; if (en) page2 = '<div class="page">' + rptHeader('PROTOCOLO GRANALLADO Y PINTURA', 'ANEXO IMAGENES EVIDENCIA', [['Documento N', p.docNro || ''], ['Orden de Trabajo', p.ot || ''], ['Pagina', '2/2']], p.logoCliente) + rptInfo(info2) + rptSection('Anexo imagenes evidencia') + ev + rptSection('Firmas') + rptSign(p.firmas) + rptFooter() + '</div>';
...
function MarcasPiezaBlock({ marcas, onChange }) {
  ...
  return (<div style={{ margin: '10px 0 4px' }}>
    ...
    {arr.map((m, i) => (<div key={i} style={{ display: 'flex', gap: 6, marginBottom: 6, alignItems: 'center' }}>
    ...
    <button onClick={() => onChange([...arr, ''])} style={{ background: C.teal, color: '#fff', border: 'none', padding: '5px 10px', cursor: 'pointer', fontSize: 11.5, marginTop: 2 }}>+ Agregar marca de pieza</button>
  </div>)
}
