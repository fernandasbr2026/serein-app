...
function rptLogo() { return '<div class="logo"><span class="lg-a">SEREIN</span><span class="lg-b">GROUP</span></div>' }
function statusBadge(v) { return v === 'SI' ? '<span class="badge badge-ok">&#10003; SI</span>' : (v === 'NO' ? '<span class="badge badge-no">&#10007; NO</span>' : esc(v)) }
function rptHeader(titulo, sub, rows) { var d = ''; rows.forEach(function (r) { d += '<div class="cb-row"><span class="cb-k">' + r[0] + '</span><span class="cb-v">' + esc(r[1]) + '</span></div>' }); return '<div class="rhead"><div class="stripe"></div>' + rptLogo() + '<div class="rh-mid"><div class="rh-title">' + titulo + '</div><div class="rh-sub">' + sub + '</div></div><div class="codebox">' + d + '</div></div>' }
...
function rptFooter() { return '<div class="rfooter"><div class="rf-navy"><span>Compromiso con la calidad</span><span>Seguridad en cada proceso</span><span>Excelencia en resultados</span></div><div class="rf-web">www.sereingroup.cl</div></div>' }
function tablaMedHTML(filas) { var nc = (filas && filas[0] ? filas[0].length : 5); var th = ['Item']; for (var a = 1; a <= nc; a++) th.push('' + a); th.push('Prom.'); var rows = []; for (var i = 0; i < filas.length; i++) { var f = filas[i]; var row = ['' + (i + 1)]; for (var j = 0; j < f.length; j++) row.push(esc(f[j] || '')); var pr = promArr(f); row.push('<b>' + (pr ? pr.toFixed(2) : '') + '</b>'); rows.push(row) } return rptTable(th, rows) }
function htmlPIG(p) {
  ...
  var perfil = rptTable(['Parametro', 'Valor', 'Parametro', 'Valor'], [['Limpieza superficial SSPC-SP', esc(p.limpiezaSSPC), 'Perfil solicitado', esc(p.perfilSolicitado)], ['Medidas rugosidad', esc((p.medidas || []).join('  -  ')), 'Promedio', promTxt], ['Perfil obtenido', esc(p.perfilObtenido || promTxt), 'Cumple', statusBadge(p.perfilCumple)]]);
  var amb = rptTable(['Parametro', 'Valor', 'Parametro', 'Valor'], [['Fecha', esc(p.amb.fecha), '% Humedad', esc(p.amb.humedad)], ['T. Ambiente C', esc(p.amb.tAmbiente), 'C Pieza', esc(p.amb.tPieza)], ['Pto. Rocio', esc(p.amb.ptoRocio), 'Hora inicio', esc(p.amb.horaInicio)]]);
  var page1 = '<div class="page">' + rptHeader('PROTOCOLO INICIO DE GRANALLA', 'SEREIN GROUP', [['Codigo', p.codigo || ''], ['Documento N', p.docNro || '']]) + rptInfo(info) + rptSection('1. Analisis proceso de granallado') + rptTable(['N', 'Control', 'Cumple', 'Observacion'], chkRows) + rptSection('2. Inspeccion de perfil') + perfil + rptSection('3. Condiciones ambientales') + amb + rptSection('4. Firmas') + rptSign(p.firmas) + rptFooter() + '</div>';
...
function htmlPGP(p) {
  ...
  var capasHtml = ''; (p.capas || []).forEach(function (cap) { var cp = (cap.filas || []).map(function (f) { return promArr(f) }); var cprom = cp.length ? (cp.reduce(function (a, b) { return a + b }, 0) / cp.length) : 0; capasHtml += rptSection('Esquema: ' + esc(cap.nombre || 'Capa') + '  -  ' + esc(cap.producto || '') + '  (Solicitado ' + esc(cap.solicitado || '') + '  /  Promedio ' + (cprom ? cprom.toFixed(2) : '') + ')') + tablaMedHTML(cap.filas || []) });
  var page1 = '<div class="page">' + rptHeader('PROTOCOLO GRANALLADO Y PINTURA', 'SEREIN GROUP', [['Codigo', p.codigo || ''], ['Documento N', p.docNro || '']]) + rptInfo(info) + rptSection('1. Instrumentos utilizados') + instr + rptSection('2. Condiciones ambientales') + amb + rptSection('3. Perfil de rugosidad (Limpieza ' + esc(p.limpiezaSSPC || '') + '  /  Solicitado ' + esc(p.perfilSolicitado || '') + '  /  Obtenido ' + (perfObt ? perfObt.toFixed(2) : '') + ')') + perfTbl + capasHtml + rptSection('Firmas') + rptSign(p.firmas) + rptFooter() + '</div>';
  ...
  return '<!doctype html><html><head><meta charset="utf-8"><title>' + esc(p.codigo || 'PGP') + '</title><style>' + PROTO_CSS + '</style></head><body>' + page1 + page2 + '</body></html>';
}
var PROTO_CSS = '@page{size:A4;margin:20mm 15mm 15mm 15mm}*{box-sizing:border-box}body{font-family:Inter,Arial,Helvetica,sans-serif;color:#101828;margin:0;-webkit-print-color-adjust:exact;print-color-adjust:exact}.page{position:relative;padding-bottom:70px}.page+.page{page-break-before:always}.rhead{position:relative;display:flex;align-items:center;justify-content:space-between;border-bottom:3px solid #061A40;padding:4px 0 10px;overflow:hidden}.logo{display:flex;align-items:baseline}.lg-a{color:#061A40;font-weight:800;font-size:22px;letter-spacing:1px}.lg-b{color:#FF6B00;font-weight:800;font-size:22px;margin-left:5px;letter-spacing:1px}.rh-mid{flex:1;text-align:center}.rh-title{color:#061A40;font-weight:800;font-size:17px;letter-spacing:.5px}.rh-sub{color:#FF6B00;font-weight:700;font-size:10px;letter-spacing:2px;margin-top:2px}.codebox{background:#061A40;color:#fff;padding:8px 12px;border-radius:6px;font-size:10px;min-width:160px}.cb-row{display:flex;justify-content:space-between;gap:12px;padding:1px 0}.cb-k{color:#9fb0cf}.cb-v{font-weight:700}.stripe{position:absolute;top:-10px;right:120px;width:60px;height:130%;background:#FF6B00;opacity:.10;transform:skewX(-22deg)}.infogrid{display:grid;grid-template-columns:1fr 1fr;gap:0 26px;margin:14px 0 4px}.info-item{display:flex;justify-content:space-between;border-bottom:1px solid #D8DCE5;padding:5px 2px;font-size:11px}.info-k{color:#5a6b85}.info-v{font-weight:700;color:#101828;text-align:right}.sec-title{color:#061A40;font-weight:800;font-size:12px;text-transform:uppercase;border-left:4px solid #FF6B00;padding-left:9px;margin:16px 0 7px;letter-spacing:.4px}table.dt{width:100%;border-collapse:collapse;font-size:11px;margin-bottom:4px}table.dt th{background:#061A40;color:#fff;padding:6px 8px;text-align:left;font-weight:700;font-size:10px}table.dt td{border:1px solid #D8DCE5;padding:6px 8px}table.dt td.c{text-align:center}.badge{display:inline-block;padding:2px 9px;border-radius:20px;font-size:10px;font-weight:700}.badge-ok{background:#e6f7ec;color:#16A34A}.badge-no{background:#fdeaea;color:#DC2626}.norm{display:inline-block;padding:1px 7px;border:1px solid #D8DCE5;border-radius:4px;font-size:9px;color:#5a6b85;background:#F5F7FA}.sign-cell{height:32px}.rfooter{display:flex;margin-top:20px;border-radius:6px;overflow:hidden;border:1px solid #D8DCE5}.rf-navy{background:#061A40;color:#fff;flex:1;display:flex;gap:20px;justify-content:center;align-items:center;padding:10px;font-size:10px;font-weight:600}.rf-web{background:#FF6B00;color:#fff;padding:0 16px;font-weight:700;font-size:11px;display:flex;align-items:center}.evcard{border:1px solid #D8DCE5;border-radius:8px;padding:12px 14px;margin-bottom:12px;display:flex;gap:16px;page-break-inside:avoid;background:#fff}.ev-left{width:36%}.ev-title{color:#FF6B00;font-weight:800;font-size:12px}.ev-desc{margin:5px 0;font-size:11px;font-weight:600;color:#101828}.ev-obs{font-size:10.5px;color:#344054;line-height:1.4}.ev-right{flex:1;display:grid;gap:6px}.ev-right.g1{grid-template-columns:1fr}.ev-right.g2{grid-template-columns:repeat(2,1fr)}.ev-right.g3{grid-template-columns:repeat(3,1fr)}.ev-right.g4{grid-template-columns:repeat(2,1fr)}.imgframe{border:2px solid #061A40;border-radius:4px;overflow:hidden;height:150px;background:#F5F7FA;display:flex;align-items:center;justify-content:center}.imgframe img{max-width:100%;max-height:100%;width:auto;height:auto;object-fit:contain;display:block}'
