...
const imgToData = (file, cb) => { const r = new FileReader(); r.onload = e => { const img = new Image(); img.onload = () => { const cv = document.createElement('canvas'); cv.width = FOTO_W; cv.height = FOTO_H; const ctx = cv.getContext('2d'); const ar = img.width / img.height, tar = FOTO_W / FOTO_H; let sw, sh, sx, sy; if (ar > tar) { sh = img.height; sw = sh * tar; sx = (img.width - sw) / 2; sy = 0 } else { sw = img.width; sh = sw / tar; sx = 0; sy = (img.height - sh) / 2 } ctx.drawImage(img, sx, sy, sw, sh, 0, 0, FOTO_W, FOTO_H); cb(cv.toDataURL('image/jpeg', 0.72)) }; img.src = e.target.result }; r.readAsDataURL(file) }
...
function htmlPIG(p) {
  ...
  var evHtml = ''; var en = 0; evChecks.forEach(function (c) { en++; var imgs = c.fotos || []; var grid = imgs.length > 3 ? 'g4' : (imgs.length === 1 ? 'g1' : 'g3'); var ig = ''; imgs.forEach(function (d) { ig += '<div class="imgframe"><img src="' + d + '"/></div>' }); evHtml += '<div class="evcard"><div class="ev-left"><div class="ev-title">EVIDENCIA ' + en + ':</div><div class="ev-desc">' + esc(c.nombre) + '</div><div class="ev-obs"><b>OBS.:</b> ' + esc(c.obs) + '</div></div><div class="ev-right ' + grid + '">' + ig + '</div></div>' });
