...
function rptInfo(pairs, equipos) { var s = '<div class="infogrid">'; pairs.forEach(function (pr) { var img = FILAS_CON_FIRMA.indexOf(pr[0]) >= 0 ? firmaImgPara(equipos, pr[1]) : ''; s += '<div class="info-item"><span class="info-k">' + pr[0] + '</span><span class="info-v">' + esc(pr[1]) + (img ? ' <img src="' + img + '" style="height:20px;vertical-align:middle;margin-left:4px"/>' : '') + '</span></div>' }); return s + '</div>' }
...
function rptSign(firmas, equipos) { var r = ''; (firmas || []).forEach(function (f) { var img = firmaImgPara(equipos, f.quien); r += '<tr><td>' + esc(f.rol) + '</td><td>' + esc(f.quien) + '</td><td class="sign-cell">' + (img ? '<img src="' + img + '" style="height:26px;display:block;margin:0 auto"/>' : '') + '</td><td>' + esc(f.fecha) + '</td></tr>' }); return '<table class="dt"><thead><tr><th>Rol</th><th>Nombre</th><th>Firma</th><th>Fecha</th></tr></thead><tbody>' + r + '</tbody></table>' }
