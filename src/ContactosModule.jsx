...
function Tabla({ titulo, icono, items, setItems, color, cols }) {
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #DFE4EA', borderTop: `3px solid ${color}` }}>
      <div style={{ padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8, borderBottom: '1px solid #DFE4EA' }}>
        <span style={{ fontFamily: SEREIN.fontDisplay, fontWeight: 600, fontSize: 14, textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: 6 }}>{icono} {titulo}</span>
      ...
      <div style={{ overflowX: 'auto', padding: 12 }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12.5 }}>
          ...
          <tbody>
            {pg.items.map(x => (
              <tr key={x.id} style={{ borderBottom: '1px solid #DFE4EA' }}>
                {cols.map(c => (
            ...
            {mostrados.length === 0 && <tr><td colSpan={cols.length + 1} style={{ padding: 14, textAlign: 'center', color: '#9AA3AD' }}>Sin registros.</td></tr>}
          </tbody>
        </table>
