...
// Sin telefono: el emisor de la OC lleva solo direccion y correo corporativo.
const _EMP_DEF_OC = { nombre: 'SERVICIOS REVESTIMIENTOS INDUSTRIALES SPA', rut: '76.860.656-0', giro: 'Revestimientos Industriales y habitacionales', dir: 'Santa Rosa 70, Lampa', email: 'administracion@sereinspa.com' }
...
const EMPRESA_OC = {}
;[['nombre', 'razonSocial'], ['rut', 'rut'], ['giro', 'giro'], ['dir', 'direccion'], ['email', 'correo']].forEach(m => Object.defineProperty(EMPRESA_OC, m[0], { get() { return _empValOC(m[0], m[1]) }, enumerable: true }))
function htmlOC(oc) {
  ...
  s += '<div class="rhead"><div class="stripe"></div>' + (function(){var _l='';try{_l=localStorage.getItem('serein_logo')||''}catch(e){}return _l?'<img src="'+_l+'" style="height:44px;display:block"/>':'<div class="logo"><span class="lg-a">SEREIN</span><span class="lg-b">GROUP</span></div>';})() + '<div class="rh-mid"><div class="rh-title">ORDEN DE COMPRA</div><div class="rh-sub">SEREIN GROUP</div></div><div class="codebox"><div class="cb-row"><span class="cb-k">Folio N</span><span class="cb-v">' + esc(oc.numero || '') + '</span></div><div class="cb-row"><span class="cb-k">Emision</span><span class="cb-v">' + esc(oc.fecha || '') + '</span></div></div></div>';
  s += '<div class="emisor"><b>' + EMPRESA_OC.nombre + '</b> - RUT ' + EMPRESA_OC.rut + ' - ' + EMPRESA_OC.giro + ' - ' + EMPRESA_OC.dir + ' - ' + EMPRESA_OC.email + '</div>';
