...
function TarjetaProyecto({ p, onUpdate, onDelete, onAddCompra, params, facturasProy = [], ppmPct = 2, enModal = false }) {
  ...
  // Estimado a pagar en TODO el proyecto (no solo lo ya cargado): ambos lados
  // se proyectan a la venta/costo TOTAL del proyecto, no solo lo ya facturado
  // o comprado — si no, se compara venta parcial real contra costo total
  // proyectado y el resultado no significa nada.
  // Venta: IVA sobre la venta cotizada completa (no solo las facturas ya emitidas).
  // Compra: usa el presupuesto (tope) de cada Centro de Costo, no el consumo
  // real, respetando los CC marcados "exento" (esos no generan crédito de IVA).
  const ivaVentaEstimado = Math.round(ventaDe(p, facturasProy) * 0.19)
  const ivaCompraEstimadoCC = Math.round(ccCodigos(p).reduce((a, id) => {
    if ((p.ccIva && p.ccIva[id]) === 'exento') return a
    return a + topeCC(p, id)
  }, 0) * 0.19)
  const ivaEstimadoPagar = ivaVentaEstimado - ivaCompraEstimadoCC
  const [abierto, setAbierto] = useState(false)
        ...
        <div style={{ borderTop: '1px solid #DFE4EA', padding: 18 }}>
          ...
          <div style={{ background: ivaEstimadoPagar >= 0 ? '#FDECDD' : '#E6F7EE', border: '1px solid ' + (ivaEstimadoPagar >= 0 ? C.ambar : '#1B9E5D'), borderRadius: 8, padding: '10px 14px', marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
            <div>
              ...
              <div style={{ fontSize: 12, color: C.gris, marginTop: 2 }}>IVA ventas (sobre venta cotizada total) {clp(ivaVentaEstimado)} − IVA compras estimado según presupuesto CC {clp(ivaCompraEstimadoCC)}</div>
              <div style={{ fontSize: 10.5, color: C.gris, marginTop: 2 }}>A diferencia del remanente de arriba (solo lo ya facturado y comprado), este proyecta TODO el proyecto: IVA sobre la venta cotizada completa y sobre el TOPE de cada Centro de Costo, respetando los CC marcados "Exento".</div>
            </div>
                      ...
                      <tr style={{ borderBottom: '1px solid #DFE4EA', background: '#F2F4F7' }}>
                        <td colSpan={10} style={{ padding: '2px 8px 8px' }}>
                          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center', fontSize: 11.5, color: '#D9600A' }}>
                            ...
                            {esManual && (<>
                              <span>Tasa <input value={ov.manualTasa != null ? ov.manualTasa : ''} onChange={ev => updFac(fx.numero, { manualTasa: ev.target.value })} placeholder="0,83" style={{ ...inp, width: 50, padding: '3px 6px', textAlign: 'right' }} /> %/mes</span>
                              <span>Tasa mora <input value={ov.manualTasaMora != null ? ov.manualTasaMora : ''} onChange={ev => updFac(fx.numero, { manualTasaMora: ev.target.value })} placeholder="3,5" style={{ ...inp, width: 50, padding: '3px 6px', textAlign: 'right' }} /> %/mes</span>
