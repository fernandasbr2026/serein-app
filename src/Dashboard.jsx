...
export default function Dashboard({ perfil, email, onLogout }) {
  ...
  const nombreTab = t => t === 'ASESOR' ? '🧠 Asesor IA' : t === 'TODAS' ? 'Consolidado' : t === 'GESTION_PROYECTOS' ? 'Proyectos' : t === 'GESTION_OT' ? 'ð§ Ãrdenes de Trabajo' : t === 'ASISTENCIA' ? 'ð· Asistencia' : t === 'FINANZAS' ? 'ð° Finanzas' : t === 'PAGOS' ? 'ðµ Proveedores y Pagos' : t === 'ORDENES_COMPRA' ? 'ð§¾ Ãrdenes de Compra' : t === 'TRAZABILIDAD' ? 'ð Trazabilidad y Alertas' : t === 'PARAMETROS' ? 'ð§® ParÃ¡metros' : t === 'CLIENTES' ? 'ð¢ Resumen ventas por cliente' : t === 'CONTACTOS' ? 'ð Clientes y Proveedores' : t === 'COTIZADOR' ? 'ð Cotizaciones' : t === 'PRODUCCION' ? 'ð­ ProducciÃ³n' : t === 'COMPRAS_OP' ? 'ð Compras Operativas' : t === 'LIBRO_COMPRAS' ? 'Libro de Compras' : t === 'LIBRO_VENTAS' ? 'Libro de Ventas' : t
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: C.niebla, fontFamily: "'Inter',sans-serif" }}>
      <aside style={{ width: 234, flexShrink: 0, background: C.carbon, display: 'flex', flexDirection: 'column', position: 'sticky', top: 0, height: '100vh' }}>
        ...
        <div style={{ margin: '0 16px 12px', fontSize: 10, color: '#6B7176', textAlign: 'center', lineHeight: 1.4 }}>
          ...
          <button onClick={() => { if (window.confirm('Â¿Borrar los datos guardados y volver a los valores base? Esta acciÃ³n no se puede deshacer.')) borrarDatosLocales() }} style={{ display: 'block', margin: '4px auto 0', background: 'transparent', border: 'none', color: '#7A8288', cursor: 'pointer', fontSize: 10, textDecoration: 'underline' }}>Restablecer datos</button>
        </div>
      </aside>
