  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #E2DED4', borderTop: '4px solid #161616', marginBottom: 16 }}>
      <div style={{ padding: '14px 18px 10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 6 }}>
        <span style={{ fontFamily: "'Oswald',sans-serif", fontWeight: 600, fontSize: 14, textTransform: 'uppercase' }}>Ã°ÂÂÂ° Resumen financiero del mes</span>
        <button onClick={onIr} style={{ background: 'none', border: '1px solid #CBD2D6', padding: '5px 12px', cursor: 'pointer', fontSize: 12 }}>Ver mÃÂ³dulo Finanzas Ã¢ÂÂ</button>
      </div>
      <div style={{ padding: '0 18px 16px', display: 'flex', gap: 28, flexWrap: 'wrap' }}>
        {item('Gastos fijos + variables', clp(r.fijos + r.variables))}
        {item('Cuotas crÃÂ©ditos/leasing', clp(r.totalCuotasMes), '#D2642F')}
...
function CardModulo({ titulo, color, abiertasN, abiertasMonto, porFacturar, facturadoPorCobrar }) {
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #E2DED4', borderTop: `4px solid ${color}`, padding: '14px 16px' }}>
      ...
      {it(`Abiertas ÃÂ· ${abiertasN} en curso`, clp(abiertasMonto) + ' por facturar', color)}
      {it('Cerradas por facturar', clp(porFacturar), '#D2642F')}
      {it('Facturado, aÃÂºn por cobrar', clp(facturadoPorCobrar), '#B5432E')}
    </div>
  )
}
...
function ResumenModulos({ ots, proyectos }) {
  ...
  const areaData = a => {
    ...
    const abiertas = list.filter(o => ['Cotizada', 'En ejecuciÃÂ³n'].includes(o.estado))
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{ fontFamily: "'Oswald',sans-serif", fontWeight: 600, fontSize: 14, textTransform: 'uppercase', marginBottom: 10 }}>OT y proyectos por mÃÂ³dulo</div>
...
export default function Dashboard({ perfil, email, onLogout }) {
  ...
  // Cada usuario ve en paralelo las OT de su ÃÂ¡rea asignada
  ...
  // Guarda automÃÂ¡ticamente en el navegador cada vez que cambian los datos
  useEffect(() => {
  ...
  // Trae la UF (valor del dÃÂ­a) al cargar la app, desde mindicador.cl (Banco Central)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    setFin(f => ({
      ...
      gastos: (f.gastos || []).map(g => {
        if (g.id === 'g1') return { ...g, uf: 180, tipo: 'fijo', categoria: 'Arriendo', nombre: 'Arriendo Santa Rosa ÃÂ· 180 UF', dist: [{ area: 'Santa Rosa', pct: 100 }], neto: Math.round(180 * ufv), iva: 0, obs: '180 UF ÃÂ $' + Math.round(ufv).toLocaleString('es-CL') + ' (vence el 5 de cada mes)' }
  ...
  // ----- Consolidado y ÃÂ¡reas: suman desde las FACTURAS consolidadas (Venta Neta) -----
  ...
  // ÃÂrdenes de compra pendientes (no Pagadas/Canceladas/Anuladas) Ã¢ÂÂ cuentas por pagar y flujo
  ...
  // Cuentas por cobrar (bruto): facturas no pagadas de las tres ÃÂ¡reas
  ...
  const otEnCurso = (ots || []).filter(o => ['Cotizada', 'En ejecuciÃÂ³n', 'Terminada'].includes(o.estado)).reduce((a, o) => a + meOT(o), 0)
  ...
  // Caja = saldo inicial + cobros registrados Ã¢ÂÂ pagos registrados (desde Finanzas/Pagos)
  ...
  // Cuentas por pagar atribuidas a un ÃÂ¡rea (para el resumen por mÃÂ³dulo)
  const pagarArea = a => {
  ...
  // Recuadro reutilizable: resumen financiero de un ÃÂ¡rea
  const resumenFinancieroArea = a => {
    ...
    return (
      <div style={{ background: '#fff', border: '1px solid #E2DED4', borderTop: `4px solid ${AREA_COLOR[a] || C.teal}`, marginBottom: 16 }}>
        <div style={{ padding: '14px 18px 6px', fontFamily: "'Oswald',sans-serif", fontWeight: 600, fontSize: 14, textTransform: 'uppercase' }}>Resumen financiero ÃÂ· {a}</div>
        <div style={{ padding: '0 18px 16px', display: 'flex', gap: 28, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          ...
          {it('Resultado (por cobrar Ã¢ÂÂ por pagar)', resultadoA, resultadoA >= 0 ? C.verde : C.rojo)}
        </div>
      </div>
    )
  }
  ...
  const nombreTab = t => t === 'ASESOR' ? 'ð§  Asesor IA' : t === 'TODAS' ? 'Consolidado' : t === 'GESTION_PROYECTOS' ? 'Proyectos' : t === 'GESTION_OT' ? 'Ã°ÂÂÂ§ ÃÂrdenes de Trabajo' : t === 'ASISTENCIA' ? 'Ã°ÂÂÂ· Asistencia' : t === 'FINANZAS' ? 'Ã°ÂÂÂ° Finanzas' : t === 'PAGOS' ? 'Ã°ÂÂÂµ Proveedores y Pagos' : t === 'ORDENES_COMPRA' ? 'Ã°ÂÂ§Â¾ ÃÂrdenes de Compra' : t === 'TRAZABILIDAD' ? 'Ã°ÂÂÂ Trazabilidad y Alertas' : t === 'PARAMETROS' ? 'Ã°ÂÂ§Â® ParÃÂ¡metros' : t === 'CLIENTES' ? 'Ã°ÂÂÂ¢ Resumen ventas por cliente' : t === 'CONTACTOS' ? 'Ã°ÂÂÂ Clientes y Proveedores' : t === 'COTIZADOR' ? 'Ã°ÂÂÂ Cotizaciones' : t === 'PRODUCCION' ? 'Ã°ÂÂÂ­ ProducciÃÂ³n' : t === 'COMPRAS_OP' ? 'Ã°ÂÂÂ Compras Operativas' : t === 'LIBRO_COMPRAS' ? 'Libro de Compras' : t === 'LIBRO_VENTAS' ? 'Libro de Ventas' : t
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: C.niebla, fontFamily: "'Inter',sans-serif" }}>
      <aside style={{ width: 234, flexShrink: 0, background: C.carbon, display: 'flex', flexDirection: 'column', position: 'sticky', top: 0, height: '100vh' }}>
        ...
        <div style={{ margin: '0 16px 12px', fontSize: 10, color: '#6B7176', textAlign: 'center', lineHeight: 1.4 }}>
          ...
          <button onClick={() => { if (window.confirm('ÃÂ¿Borrar los datos guardados y volver a los valores base? Esta acciÃÂ³n no se puede deshacer.')) borrarDatosLocales() }} style={{ display: 'block', margin: '4px auto 0', background: 'transparent', border: 'none', color: '#7A8288', cursor: 'pointer', fontSize: 10, textDecoration: 'underline' }}>Restablecer datos</button>
        </div>
      </aside>
      ...
      <div style={{ padding: 20, maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 16 }}>
          ...
          <Kpi label="PÃÂ©rdida Factoring" valor={clp(kPerd)} sub={kVenta > 0 ? `${((kPerd / kVenta) * 100).toFixed(2)}% s/ venta` : 'Ã¢ÂÂ'} color={C.ambar} icon={Landmark} />
          ...
          <>
            <div style={{ background: '#fff', border: '1px solid #E2DED4', borderTop: `4px solid ${C.verde}`, marginBottom: 16 }}>
              <div style={{ padding: '14px 18px 6px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 6 }}>
                <span style={{ fontFamily: "'Oswald',sans-serif", fontWeight: 600, fontSize: 14, textTransform: 'uppercase' }}>Ã°ÂÂÂ Resumen financiero total</span>
                <span style={{ fontSize: 11, color: '#7A8288' }}>caja + por cobrar Ã¢ÂÂ por pagar + OT en curso ÃÂ· montos con IVA</span>
              </div>
              <div style={{ padding: '0 18px 16px', display: 'flex', gap: 28, flexWrap: 'wrap', alignItems: 'flex-end' }}>
                ...
                <div style={{ borderLeft: '2px solid #E2DED4', paddingLeft: 20 }}>
                  {flujoItem('PosiciÃÂ³n financiera', clp(posicionFin), posicionFin >= 0 ? C.verde : C.rojo)}
                </div>
              </div>
            </div>
            ...
            <div style={{ background: '#fff', border: '1px solid #E2DED4', borderTop: `4px solid ${C.verde}`, marginBottom: 16 }}>
              <div style={{ padding: '14px 18px 6px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 6 }}>
                <span style={{ fontFamily: "'Oswald',sans-serif", fontWeight: 600, fontSize: 14, textTransform: 'uppercase' }}>Ã°ÂÂÂµ Flujo de caja proyectado ÃÂ· todas las ÃÂ¡reas</span>
                <span style={{ fontSize: 11, color: '#7A8288' }}>pagos: gastos + cuotas + proveedores ÃÂ· ingresos: cobros + facturas por cobrar</span>
              </div>
              <div style={{ padding: '0 18px 16px', display: 'flex', gap: 28, flexWrap: 'wrap', alignItems: 'flex-end' }}>
                {flujoItem('Total a pagar', clp(totalPagar), C.rojo)}
                {flujoItem('Vence en 7 dÃÂ­as', clp(pagar7), C.ambar)}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 16, marginBottom: 16 }}>
          <div style={{ gridColumn: 'span 1' }}>
            <Panel title={`Venta neta por mes ÃÂ· ${areaSel === 'TODAS' ? 'consolidado' : areaSel}`}>
        ...
        {esGerencia && areaSel === 'TODAS' && (
          <div style={{ marginBottom: 16 }}>
            <Panel title="Venta por ÃÂ¡rea (facturas)">
              <ResponsiveContainer width="100%" height={240}>
          ...
          <>
            ...
            <Panel title="Cobranza atrasada" right={<span style={{ fontSize: 12, color: C.rojo }}>{vista.atrasadas.length} facturas</span>}>
              {vista.atrasadas.length === 0 ? (
                <div style={{ color: C.verde, fontSize: 14, padding: '10px 0' }}>Ã¢ÂÂ Sin facturas atrasadas en esta ÃÂ¡rea.</div>
              ) : (
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                    <thead>
                      <tr style={{ borderBottom: `2px solid ${C.carbon}` }}>
                        ...
                        <th style={{ textAlign: 'right', padding: '6px 8px', fontSize: 11, color: '#7A8288', textTransform: 'uppercase' }}>DÃÂ­as</th>
                      </tr>
                    </thead>
                    <tbody>
        ...
        <div style={{ textAlign: 'center', color: '#9AA0A6', fontSize: 11, marginTop: 20 }}>
          SEREIN SpA ÃÂ· Datos: VENTAS_SEREIN_SPA_2026
        </div>
      </div>
      </main>
    </div>
