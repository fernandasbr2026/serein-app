...
function SeccionFactoring({ params, setParams }) {
  ...
  async function agregar() {
    ...
    try { await pullState() } catch (e) {}
    let fresco = null
    try { fresco = JSON.parse(localStorage.getItem('serein_params') || 'null') } catch (e) {}
    const baseParams = fresco && typeof fresco === 'object' ? fresco : params
    const baseLista = baseParams.factoring || []
    const nuevoParams = { ...baseParams, factoring: [...baseLista, { id: 'f' + Date.now(), nombre: nuevo.nombre, tasa: dec(nuevo.tasa), tasaMora: dec(nuevo.tasaMora), costoOp: num(nuevo.costoOp) }] }
  ...
  async function eliminar(id) {
    if (!window.confirm('¿Eliminar esta empresa de factoring?')) return
    try { await pullState() } catch (e) {}
    let fresco = null
    try { fresco = JSON.parse(localStorage.getItem('serein_params') || 'null') } catch (e) {}
    const baseParams = fresco && typeof fresco === 'object' ? fresco : params
    const baseLista = baseParams.factoring || []
    const nuevoParams = { ...baseParams, factoring: baseLista.filter(f => f.id !== id) }
    try { localStorage.setItem('serein_params', JSON.stringify(nuevoParams)) } catch (e) {}
    setParams(nuevoParams)
    pushState()
  }
...
// archivo tal cual al bucket "fotos-ot" de Storage y entrega el link
// firmado — mismo bucket que ya usan las fotos de OT/protocolos.
const subirCertificado = async (file, carpeta, cb) => {
  try {
    const nombre = `certificados-instrumentos/${carpeta}/${Date.now()}-${file.name.replace(/[^a-zA-Z0-9.\-_]/g, '_')}`
    const { error } = await supabase.storage.from('fotos-ot').upload(nombre, file, { contentType: file.type || 'application/octet-stream', upsert: false })
    if (error) { window.alert('No se pudo subir el certificado: ' + error.message); return }
    const { data } = await supabase.storage.from('fotos-ot').createSignedUrl(nombre, 60 * 60 * 24 * 365 * 5)
    if (data && data.signedUrl) cb(data.signedUrl)
  } catch (e) { window.alert('No se pudo subir el certificado: ' + ((e && e.message) || String(e))) }
}
...
<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px,1fr))', gap: 14 }}>
  {[['Medidor de espesor', 'espCertificado'], ['Rugosimetro', 'rugCertificado'], ['Termohigrometro', 'termoCertificado'], ['Granalla', 'granallaCertificado'], ['Galgas de calibracion', 'galgasCertificado'], ['Equipo de adherencia', 'adhCertificado']].map(eq => {
    const url = inst[eq[1]] || ''
    return (<div key={eq[1]} style={{ border: '1px solid #DFE4EA', padding: 10 }}>
      <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 6 }}>{eq[0]}</div>
      {url ? (
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          <a href={url} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: '#0E7A8F' }}>Ver certificado</a>
          <button onClick={() => set(eq[1], '')} style={{ background: 'transparent', border: '1px solid #DFE4EA', color: '#C5453D', fontSize: 11, padding: '4px 8px', borderRadius: 4, cursor: 'pointer' }}>Quitar</button>
        </div>
      ) : (
...
export default function ParametrosModule({ params: pExt, setParams: setPExt }) {
  ...
  const tabs = [{ id: 'factoring', label: 'Factoring', icono: <Landmark size={13} /> }, { id: 'uf', label: 'Valor UF', icono: <TrendingUp size={13} /> }, { id: 'instrumentos', label: 'Instrumentos', icono: <Info size={13} /> }, { id: 'empresa', label: 'Datos empresa', icono: <Landmark size={13} /> }]
  ...
  return (
    <div>
      ...
      {tab === 'empresa' && <SeccionEmpresa params={params} setParams={setParams} />}
    </div>
  )
}
